...
bool isDigit(char power) {
    string s = "0123456789";
    unordered_set<int> digits(s.begin(), s.end());

    return digits.find(power) != digits.end();
}

void solve() {
    // g++ -std=c++17 task.cpp -o task && ./task < input.txt > output.txt
    ...
    if (s.size() == 3 && s[0] == '1' && s[1] == '0' 
        && isDigit(s[2]) && s[2] != '1' && s[2] != '0') {
        cout << "YES" << nl;
        return;
    } else if (s[0] == '1' && s[1] == '0' && s[2] != '0' && isDigit(s[3])) {
        cout << "YES" << nl;
        return;
    }
